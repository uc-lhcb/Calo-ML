...
# =================
# Train VAE (Will's)
# This may or may not work for VAE AND VQVAE - it was written for VQVAE. I suspect minimal changes will be required for functionality with VAEs. 
# =================
...
def train_VAE(epoch, loader, model, optimizer, device, latent_loss_weight, train=True):
    ...
    for i in iterator:
        ...
        # convert energy to percentages. we multiply by 100 for numerical stability - peak values will be on the order of ~15 instead of .15
        images = 100*images / images.sum(dim=(1, 2, 3), keepdim=True)
        
        out, pred_props, segmentation_mask, latent_loss = model(images)
        
        # this allows us to only compute loss for bins that have energy. this will let us compute a smoother predicted distribution, which we are biased towards anyway.
        out = out*bool_mask
        # see above. convert to percentages, multiply by 100 for numerical stability
        out = 100*out / out.sum(dim=(1, 2, 3), keepdim=True)
#         out = out*bool_mask / out.sum(dim=(1, 2, 3), keepdim=True)

        # used for computing loss with total energy. the current un-commented code above computes loss with respect to percents.
        pred_positions, pred_momentum = pred_props[:, :2], pred_props[:, 2:]
        
        # Compute longitudual cluster asymmetry. You can compute loss with respect to this, but i highly do not recommend it. Left for posterity
        ...
        # Compute transverse cluster asymmetry. You can compute loss with respect to this, but i highly do not recommend it. Left for posterity
        ...
        # momentum fraction loss - px/pz. You can compute loss with respect to this, but i highly do not recommend it. Left for posterity
        pxpz_loss = F.mse_loss(pred_momentum[:,0] / pred_momentum[:,2], momentum[:,0] / momentum[:,2])
        
        # momentum fraction loss - py/pz. You can compute loss with respect to this, but i highly do not recommend it. Left for posterity
        ...
        # in case you would like to weight loss higher for bins far from the center of the event
...
#         recon_loss = (polar_loss_weights*SSE)[bool_mask]/bool_mask.float().sum()
        ...
        # again - only compute loss for bins which contain energy
        recon_loss = SSE[bool_mask]/bool_mask.float().sum()
        mse = F.mse_loss(out[bool_mask], images[bool_mask])
        
        # for computing the mask loss - we definitely want to weight with respect to distance to bin center
...
#         train_dict['Metric: RMS width'] += .mean().item()
        ...
        # This bit of code here generates samples during training, so you can monitor progress visually. This changes very frequently so I did not bother commenting.
        # If you would like to see regular samples, just paste whatever code you are using for inference. 
        if ((i+1) % 300 == 0) & train:
            model.eval()
